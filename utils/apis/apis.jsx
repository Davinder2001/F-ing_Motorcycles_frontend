...
export const EXPORT_ALL_APIS = () => {
  ...
  const fetchGreenEnergy = async () => {
    let resp = await fetch(`${API_URL}/api/green-energy`);
    let data = await resp.json();
    return data;
  };
  const fetchGreenEnergyGalry = async () => {
    let resp = await fetch(`${API_URL}/api/green-energy-gallery`);
    let data = await resp.json();
    return data;
  };
  ...
  const updateGallery = async (token, gallerySeoId, data) => {
    ...
    try {
      ...
      // Make the API call
      const response = await fetch(
        `${API_URL}/api/homeGallery/${gallerySeoId}`,
        {
  ...
  const createGreenEnergyGallery = async (token, content) => {
    ...
    const response = await fetch(`${API_URL}/green-energy-gallery`, {
      ...
      body: newFormData, // Pass FormData as body
    });
  
    // Check for response status
    if (!response.ok) {
      throw new Error("Failed to create Green Energy Gallery");
    }
  
    alert("Green Energy Gallery added successfully!");
    return response.json();
  };
  

  const updateGreenEnergyGallery = async (token, galleryId, data) => {
    try {
      const formData = new FormData();

      // Append the heading field
      formData.append("heading", data.imgHeading);

      // Check if an image is being updated, and append it if it's valid
      if (
        data.image &&
        data.image instanceof File &&
        data.image.type.startsWith("image/")
      ...
      // Append the `_method` field to indicate a PUT request
      formData.append("_method", "PUT");

      // Make the API call
      const response = await fetch(
        `${API_URL}/api/green-energy-gallery/${galleryId}`,
        {
          method: "POST", // Using POST for FormData with the _method override
          headers: {
            Authorization: `Bearer ${token}`,
          },
          body: formData,
      ...
      if (!response.ok) {
        throw new Error("Failed to update Green Energy Gallery");
      }

      alert("Green Energy Gallery updated successfully!");
      return response.json();
    } catch (error) {
      console.error("Error updating gallery:", error.message);
      alert("Failed to update Green Energy Gallery.");
    }
  };
  const deleteGreenEnergyGallery = async (token, galleryId) => {
    const response = await fetch(
      `${API_URL}/api/green-energy-gallery/${galleryId}`,
      {
        method: "DELETE",
    ...
    if (!response.ok) {
      throw new Error("Failed to delete Green Energy Gallery");
    }

    alert("Green Energy Gallery deleted successfully!");
    return response.json();
  };

  const createGreenEnergy = async (token, formData) => {
    // Create a new FormData object
    const newFormData = new FormData();

    // Append the form data fields to the new FormData object
    ...
    newFormData.append("image", formData.image); // Assuming image is a file input or a file object

    console.log("New FormData", newFormData);

    // Send the FormData in the body
    const response = await fetch(`${API_URL}/api/green-energy`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        // No need to set 'Content-Type', fetch will handle it automatically for FormData
      },
      body: newFormData,
    });
    ...
    console.log("Res", response);
    if (!response.ok) {
      throw new Error("Failed to create Green Energy record");
    }

    alert("Green Energy record added successfully!");
    return response.json();
  };

  const updateGreenEnergy = async (token, energyId, data) => {
    try {
      const formData = new FormData();
      ...
      formData.append("_method", "PUT"); // Using PUT override

      const response = await fetch(`${API_URL}/api/green-energy/${energyId}`, {
        method: "POST", // Using POST for FormData with the _method override
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!response.ok) {
        throw new Error("Failed to update Green Energy record");
      }

      alert("Green Energy record updated successfully!");
      return response.json();
    } catch (error) {
      console.error("Error updating record:", error.message);
      alert("Failed to update Green Energy record.");
    }
  };

  const deleteGreenEnergy = async (token, energyId) => {
    const response = await fetch(`${API_URL}/api/green-energy/${energyId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      throw new Error("Failed to delete Green Energy record");
    }

    alert("Green Energy record deleted successfully!");
    return response.json();
  };
  ...
  return {
    ...
    updateGallery,
    deleteGallery,
    createGreenEnergyGallery,
    updateGreenEnergyGallery,
    deleteGreenEnergyGallery,
    createGreenEnergy,
    updateGreenEnergy,
    deleteGreenEnergy,
  };
